            ...
            if (found) {
                ...
                for (let m = 0; m < board.length; m++) {
                    ...
                    for (let n = 0; n < board[0].length; n++) {
                      if (found && tofind[m - i] && tofind[m - i][n - j] !== ' ' &&  i <= m &&  m < i + tofind.length && j <= n &&  n < j + tofind[0].length) {
                        row += board[m][n];
                      } else {
                        row += '-';
                      }
...
function getArgument() {
    const board = fs.readFileSync('board.txt', 'utf-8').trim().split('\n').map(row => row.split(''));
    const toFind = fs.readFileSync(process.argv[3], 'utf-8').trim().split('\n').map(row => row.split(''));
    return { board, toFind };
}
